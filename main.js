...
var author$project$Exposition$getText = function (nodes) {
	getText:
	while (true) {
		if (nodes.b) {
			switch (nodes.a.$) {
				case 'Text':
					...
					return A2(
						...
						t,
						author$project$Exposition$getText(rest));
				case 'Comment':
					var rest = nodes.b;
					var $temp$nodes = rest;
					nodes = $temp$nodes;
...
var author$project$Exposition$findHeaders = function (node) {
	if (node.$ === 'Element') {
		switch (node.a) {
			case 'h1':
				var attr = node.b;
			...
			default:
				var children = node.c;
				return A2(elm$core$List$concatMap, author$project$Exposition$findHeaders, children);
		}
	} else {
		return _List_Nil;
	}
...
var author$project$Exposition$createToc = function (expo) {
	var _n0 = hecrj$html_parser$Html$Parser$run(expo.renderedHtml);
	if (_n0.$ === 'Ok') {
		var html = _n0.a;
		return A2(elm$core$List$concatMap, author$project$Exposition$findHeaders, html);
	} else {
		return _List_Nil;
	}
};
var author$project$Exposition$updateToc = function (expo) {
	return _Utils_update(
		expo,
		{
			toc: author$project$Exposition$createToc(expo)
		});
};
	...
	function (msg, model) {
		...
		while (true) {
			switch (msg.$) {
				...
				case 'SaveExposition':
					var modelWithToc = _Utils_update(
						model,
						{
					...
					return _Utils_Tuple2(
						modelWithToc,
						A2(author$project$RCAPI$saveExposition, modelWithToc.exposition, author$project$Main$SavedExposition));
...
var author$project$Main$mkDropdown = F4(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				A4(
				author$project$Main$mkDropdown,
